...
public :
   ...
   Float_t SF_topBJ;
   Float_t SF_topBJ_errUp;
   Float_t SF_topBJ_errDn;
   Float_t SF_top2BJs;
   ...
   virtual Bool_t   FillHistograms(int i, int j, int k, int l, int m, double mt, double lep_pt, double weight);
     ...
     for(unsigned int k=0; k<QCD.size(); ++k){
      for(unsigned int l=0; l<Syst.size(); ++l){
       for(unsigned int m=0; m<WFlav.size(); ++m){
        ...
        hists_goodLep_pt[i][j][k][l][m] = TH1F(histoname_goodEle_pt, "Leading Lepton pT", 40, 0., 200.);
...
Bool_t histoFiller::FillHistograms(int i, int j, int k, int l, int m, double mt, double lep_pt, double weight){
 ...
 hists_goodJ1J2_mass[i][j][k][l][m].Fill(goodJ1_mass+goodJ2_mass,weight);
...
//  Bool_t *isWl, Bool_t *isWc, Bool_t *isWcc, Bool_t *isWbb,
//  int pspace_nr, int syst_nr,
//  // variables
//  Float_t *mt_mu_good, Float_t *mt_mu_qcd, Float_t *mt_ele_good, Float_t *mt_ele_qcd,
//  // weights
//  Float_t *weight_mu_good, Float_t *weight_mu_qcd, Float_t *weight_ele_good, Float_t *weight_ele_qcd
//)
Bool_t histoFiller::fillHistWriter(
  Bool_t pass_mu_good, Bool_t pass_mu_qcd, Bool_t pass_ele_good, Bool_t pass_ele_qcd,
  int pspace_nr, int syst_nr,
...
{
 if( pass_mu_good )          { FillHistograms(pspace_nr,0,0,syst_nr,0,mt_mu_good_val,lep_mu_good_pt,weight_mu_good_val); } 
 if( pass_mu_good && isWl )  { FillHistograms(pspace_nr,0,0,syst_nr,1,mt_mu_good_val,lep_mu_good_pt,weight_mu_good_val); } 
 if( pass_mu_good && isWc )  { FillHistograms(pspace_nr,0,0,syst_nr,2,mt_mu_good_val,lep_mu_good_pt,weight_mu_good_val); } 
 if( pass_mu_good && isWcc ) { FillHistograms(pspace_nr,0,0,syst_nr,3,mt_mu_good_val,lep_mu_good_pt,weight_mu_good_val); } 
 if( pass_mu_good && isWbb ) { FillHistograms(pspace_nr,0,0,syst_nr,4,mt_mu_good_val,lep_mu_good_pt,weight_mu_good_val); } 
 if( pass_mu_qcd )           { FillHistograms(pspace_nr,0,1,syst_nr,0,mt_mu_qcd_val,lep_mu_qcd_pt,weight_mu_qcd_val); } 
 if( pass_mu_qcd && isWl )   { FillHistograms(pspace_nr,0,1,syst_nr,1,mt_mu_qcd_val,lep_mu_qcd_pt,weight_mu_qcd_val); } 
 if( pass_mu_qcd && isWc )   { FillHistograms(pspace_nr,0,1,syst_nr,2,mt_mu_qcd_val,lep_mu_qcd_pt,weight_mu_qcd_val); } 
 if( pass_mu_qcd && isWcc )  { FillHistograms(pspace_nr,0,1,syst_nr,3,mt_mu_qcd_val,lep_mu_qcd_pt,weight_mu_qcd_val); } 
 if( pass_mu_qcd && isWbb )  { FillHistograms(pspace_nr,0,1,syst_nr,4,mt_mu_qcd_val,lep_mu_qcd_pt,weight_mu_qcd_val); }    
 if( pass_ele_good )          { FillHistograms(pspace_nr,1,0,syst_nr,0,mt_ele_good_val,lep_ele_good_pt,weight_ele_good_val); }    
 if( pass_ele_good && isWl )  { FillHistograms(pspace_nr,1,0,syst_nr,1,mt_ele_good_val,lep_ele_good_pt,weight_ele_good_val); }    
 if( pass_ele_good && isWc )  { FillHistograms(pspace_nr,1,0,syst_nr,2,mt_ele_good_val,lep_ele_good_pt,weight_ele_good_val); }    
 if( pass_ele_good && isWcc ) { FillHistograms(pspace_nr,1,0,syst_nr,3,mt_ele_good_val,lep_ele_good_pt,weight_ele_good_val); }    
 if( pass_ele_good && isWbb ) { FillHistograms(pspace_nr,1,0,syst_nr,4,mt_ele_good_val,lep_ele_good_pt,weight_ele_good_val); }    
 if( pass_ele_qcd )           { FillHistograms(pspace_nr,1,1,syst_nr,0,mt_ele_qcd_val,lep_ele_qcd_pt,weight_ele_qcd_val); }      
 if( pass_ele_qcd && isWl )   { FillHistograms(pspace_nr,1,1,syst_nr,1,mt_ele_qcd_val,lep_ele_qcd_pt,weight_ele_qcd_val); }      
 if( pass_ele_qcd && isWc )   { FillHistograms(pspace_nr,1,1,syst_nr,2,mt_ele_qcd_val,lep_ele_qcd_pt,weight_ele_qcd_val); }      
 if( pass_ele_qcd && isWcc )  { FillHistograms(pspace_nr,1,1,syst_nr,3,mt_ele_qcd_val,lep_ele_qcd_pt,weight_ele_qcd_val); }      
 if( pass_ele_qcd && isWbb )  { FillHistograms(pspace_nr,1,1,syst_nr,4,mt_ele_qcd_val,lep_ele_qcd_pt,weight_ele_qcd_val); }      
 return kTRUE;
}
