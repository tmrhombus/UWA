...
void MuNuPlot2(TString var,bool log,bool save,char* savefile, int bins, double lo, double hi, double top, TString cutInput,char* notes1,TString weightinput, double QCDNORM, bool setLog){
...
TH1F *  Wbb = createhist("Wbb",var,"Wbig.root",dirW,kBlue-10,weightW+"*__WEIGHTPU2__*__WEIGHTLHE__*0.000165*0.566842",false,cuts+"&&"+bHadron+"&&"+cutInput,bins,lo,hi);//1.571
  //Wbb2 = createhist("Wbb2",var,"W.root",dir,kAzure+1,weight+"*1",false,cuts+"&&"+bHadron+"&&"+cutInput,bins,lo,hi);
  //Wbbf = createhist("Wbbf",var,"Wbig.root",dirW,kBlue-10,"1",false,cuts+"&&"+bHadron+"&&"+cutInput,bins,lo,hi);//1.571
TH1F *  Wc = createhist("Wc",var,"Wbig.root",dirW,kMagenta-4,weightW+"*__WEIGHTPU2__*__WEIGHTLHE__*0.000165*0.580816",false,cuts+"&&!"+bHadron+"&&(ncCands+ncbarCands>0)&&!"+Wcvareven+"&&"+cutInput,bins,lo,hi);//1.571
  //Wcf = createhist("Wcf",var,"Wbig.root",dirW,kMagenta-4,"1",false,cuts+"&&!"+bHadron+"&&(ncCands+ncbarCands>0)&&!"+Wcvareven+"&&"+cutInput,bins,lo,hi);//1.571
  //Wccf = createhist("Wccf",var,"Wbig.root",dirW,kViolet-6,"1",false,cuts+"&&!"+bHadron+"&&(ncCands+ncbarCands>0)&&"+Wcvareven+"&&"+cutInput,bins,lo,hi);//1.571
...
//Wcc2 = createhist("Wcc2",var,"W.root",dir,kAzure+1,weight+"*1",false,cuts+"&&!"+bHadron+"&&(ncCands+ncbarCands>0)&&"+Wcvareven+"&&"+cutInput,bins,lo,hi);
...
TH1F *  tW = createhist("tW",var,"TtWDR.root",dir,kOrange+7,weight,false,cuts+"&&"+cutInput,bins,lo,hi);
  ...
  //qcd estimate
  TH1F *Wq = createhist("Wq",var,"Wbig.root",dirW,kAzure+1,weightW+"*__WEIGHTPU2__*__WEIGHTLHE__*0.000165*0.566842",false,cutsq+"&&"+cutInput,bins,lo,hi);//1.571
  ...
  Den->Add(W);Den->Add(WZ);Den->Add(WW);
  Den->Add(ZJets);Den->Add(QCD);Den->Add(T);Den->Add(Tbar);Den->Add(tW);Den->Add(TOP);Den->Add(Wbb);Den->Add(Wc);Den->Add(Wcc);//Den->Add(Wb2);
  Num->Add(Data);
  ...
  if(save){
    Tcan->SaveAs(TString::Format("~/www/%s.png",savefile));
    Tcan->SaveAs(TString::Format("~/www/%s.C",savefile));

	TFile *f1= new TFile(TString::Format("~/www/%s.root",savefile), "RECREATE");
        W->SetName("W"); W->Write(); 
        Wbb->SetName("Wbb"); Wbb->Write();
        Wcc->SetName("Wcc"); Wcc->Write();
        Wc->SetName("Wc"); Wc->Write();
        TOP->SetName("ttbar"); TOP->Write();
        ZJets->SetName("ZJets"); ZJets->Write();
        WW->SetName("WW"); WW->Write();
        WZ->SetName("WZ"); WZ->Write();
        T->SetName("T"); T->Write();
        tW->SetName("tW"); tW->Write();
        Tbar->SetName("Tbar"); Tbar->Write();
        QCD->SetName("QCD"); QCD->Write();
...
//  TString treeName = "muNuEventTreeFIT/eventTree";
  ...
  TH1F *tempHist = new TH1F("tempName","hist",bins,lo,hi); //tempHist->Sumw2();
