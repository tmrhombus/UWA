cut = 'C2'
andreas_events = [line.strip() for line in open('andrea_%s_tt.txt'%(cut))]
jelenas_events = [line.strip() for line in open('jelena_%s_tt.txt'%(cut))]
toms_events    = [line.strip() for line in open('tom_%s_tt.txt'%(cut))]
...
log = open('/afs/hep.wisc.edu/home/tperry/www/wbb/synch/2014sptr10/mc/%s_comp.log'%(cut),'w')
