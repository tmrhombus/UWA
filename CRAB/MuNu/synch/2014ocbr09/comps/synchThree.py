cut = 'C1'
andreas_events = set([line.strip() for line in open('%s_and.txt'%(cut))])
jelenas_events = set([line.strip() for line in open('%s_jel.txt'%(cut))])
toms_events    = set([line.strip() for line in open('%s_tom.txt'%(cut))])
...
log = open('/afs/hep.wisc.edu/home/tperry/www/wbb/synch/2014ocbr13/%s_tt_m12e10_comp.log'%(cut),'w')
