...
tfile.Get("data_obs_Wi").Draw("hist")
sf_nToGiu = gfile.Get("data_obs_Tr").Integral(-1,-1) / tfile.Get("data_obs_Wi").Integral(-1,-1) 
sf_qToTom = tfile.Get( qcd+"_Wi" ).Integral(-1,-1) / gfile.Get( qcd+"_Tr" ).Integral(-1,-1) 
...
for hname in histnames:
 ...
 if not (
  ...
  fnm.fnmatch(hname,"*qcd*") ) : htom.Scale(stom)
 ...
 if not (
  fnm.fnmatch(hname,"data_obs") or
  fnm.fnmatch(hname,"*qcd*") ) : hgiu.Scale(sgiu)
