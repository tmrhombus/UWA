...
version_mc="Schweincomp"
runname_mc="wbb_csvt_mt0_v4"
version_data="Schweincomp"
runname_data="wbb_csvt_mt0_v4"
...
sf_qcds_mu =  [0.133873483848,]
sf_qcds_ele = [0.113973988715,]



########## Schweincomp wbb_csvm_mt0_v2
#sf_qcds_mu =  [0.054508262844,0.0001,0.0980109879977,1.,1.,1.,1.]
#sf_qcds_ele = [0.0714587028442,0.0001,0.0915771015454,1.,1.,1.,1.,1.]
#sf_qcds_mu =  [0.319560350014,]
#sf_qcds_ele = [0.122451288395,]

######### Q,Dercury wbb_csvm_mt0_counting
#sf_qcds_mu =  [0.0556413670459,0.0001,0.104013589574,]#0.318304661955,0.205666055408,0.0001,0.0001]
#sf_qcds_ele = [0.0950068791002,0.0001,0.122187933874,]#0.198909209394,0.231806577237,0.0001,0.0001]

######## Q,Dercury wbb_csvm_mt0
#sf_qcds_mu =  [0.0565799756293,0.0001,0.104273502514,]#0.318304661955,0.205666055408,0.0001,0.0001]
#sf_qcds_ele = [0.0947419108055,0.0001,0.122017930223,]#0.198909209394,0.231806577237,0.0001,0.0001]

######## Earth wbb_csvm_mt0
#sf_qcds_mu =  [0.0550835684438,0.0001,0.0697313207330,0.320093544207,0.403397757245,0.0001,0.0001]
#sf_qcds_ele = [0.0789739676887,0.0001,0.0868923589546,0.147700724183,0.357441003713,0.0001,0.0001]
...
for lep in leps:
 ...
 for variable in variables: 
  for sf_qcd,cut in zip(sf_qcds,cuts):
   ...
   for key in inFile.GetListOfKeys():
    ...
    if(obj.IsA().InheritsFrom("TH1")):
     ...
     if( fnm.fnmatch( h_name,"h_%s_%s_good_%s"%(cut,lep,variable) ) ):
      ...
      # if it's "good" and mT, get ready for qcd subtraction
      if variable=="mt":
      ...
      outfile.cd()
      obj.Write()
     # if histogram is "qcd" do the same thing
   ...
   #### MC ####
   mc_good5F_mt020 = 0. # total mc in 0<mt<20
         ...
         if(obj.IsA().InheritsFrom("TH1")):
          ...
          if( fnm.fnmatch(h_name,"h_%s_%s_good_%s%s%s"%(cut,lep,variable,sf_shift,q_flavor)) ):
           ...
           if( fnm.fnmatch(sf_shift,"") 
            ...
            mc_good5F_mt020 += obj.Integral(bmin,bmax)
   ...
   if variable=="mt":
    if( mc_qcd_mt020 > 0.1 ):
     log.write("\n%s  : QCD Scale = ( data - mc )/qcd in 0<mt<20 \n" \
          %( (data_good_mt020 - mc_good5F_mt020)/mc_qcd_mt020 ) )
