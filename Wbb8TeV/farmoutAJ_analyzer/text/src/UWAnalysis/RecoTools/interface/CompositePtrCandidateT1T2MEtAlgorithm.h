...
//    (depending on recoMode configuration parameter)
    ...
    //Set Higgs Mass Using Mjj Method
    reco::Candidate::LorentzVector Higgs(0,0,0,0);
    reco::Candidate::LorentzVector HiggsSV(0,0,0,0); 

    //Set Higgs Mass Using SV Mass Method
    math::PtEtaPhiMLorentzVector SV1(0,0,0,0);
	 ...
	 HiggsSV = leg1->p4()+leg2->p4()+SV1+SV2;
	 Higgs = leg1->p4()+leg2->p4()+cleanedJets.at(0)->p4()+cleanedJets.at(1)->p4();
      }

    compositePtrCandidate.setHiggs(Higgs);
    compositePtrCandidate.setHiggsSV(HiggsSV);
