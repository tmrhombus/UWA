...
class CompositePtrCandidateTMEtProducer : public edm::EDProducer 
{
 ...
 public:
    ...
    : algorithm_()
  {
    ...
    srcJets_ = cfg.getParameter<edm::InputTag>("srcJets");//srcJets
    ...
    SimBHadronCollection_ = cfg.getParameter<edm::InputTag>("SimBHadronCollection");
  ...
  void produce(edm::Event& evt, const edm::EventSetup& es)
  {
   ...
   edm::Handle<TView> visDecayProductsCollection;
    ...
    //edm::InputTag bhadrons_;
    bhadronPtr bhadrons;
    edm::Handle<std::vector<SimBHadron> > SimBHadronCollection;
    if(evt.getByLabel(SimBHadronCollection_,SimBHadronCollection)){
    for(unsigned int i=0;i<SimBHadronCollection->size();++i)
      //pfJets.push_back(jetCollection->ptrAt(i));
      bhadrons.push_back(SimBHadronCollection->at(i));
    }
	...
	CompositePtrCandidateTMEt<T> compositePtrCandidate = 
	  algorithm_.buildCompositePtrCandidate(visDecayProductsPtr, metPtr,pfJets,*visDecayProductsCollection,genParticles,bhadrons);
 ...
 private:
  ...
  edm::InputTag SimBHadronCollection_;  
