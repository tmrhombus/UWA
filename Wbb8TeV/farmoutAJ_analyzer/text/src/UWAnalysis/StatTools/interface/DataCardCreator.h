...
class DataCardCreator {
 public:
  ...
  DataCardCreator(optutl::CommandLineParser parser) {
    ...
    vbfQCDSelection_ = parser.stringValue("vbfQCDSelection");
...
//    fprintf(pfile,"CMS_scale_j           lnN  0.96        0.99          0.92          -            -              0.94            0.97    Jet scale\n");
  ...
  void makeHiggsShapesSM(std::string preselection,std::string prefix) {
    ...
    for(unsigned int i=0;i<smMasses_.size();++i) {
	  ...
	  if(prefix=="_SM1" ||prefix=="_SM3"||prefix=="_SM2")
	    tmp= createHistogramAndShiftsFinal(dir_+"sm"+smMasses_[i]+".root","ggH"+smMasses_[i],("("+preselection+"&&"+trigSelection_+"&&"+osSignalSelection_+")*0.6*"+weight_),luminosity_*legCorr*smSigma_[i],prefix);
	  else
	    tmp= createHistogramAndShiftsFinal(dir_+"sm"+smMasses_[i]+".root","ggH"+smMasses_[i],("("+preselection+"&&"+trigSelection_+"&&"+osSignalSelection_+")*"+weight_),luminosity_*legCorr*smSigma_[i],prefix);
	  }
      tmp= createHistogramAndShiftsFinal(dir_+"vbf"+smMasses_[i]+".root","qqH"+smMasses_[i],("("+preselection+"&&"+trigSelection_+"&&"+osSignalSelection_+")*"+weight_),luminosity_*legCorr*vbfSigma_[i],prefix);
  ...
  BkgOutput runOSLSMT(std::string preSelection,std::string prefix,std::string zShape) {
    ...
    //create Z->tautau 
    std::pair<float,float> zttYield       = createHistogramAndShifts(zttFile_,"ZTTTMP",("("+preSelection+"&&"+trigSelection_+"&&"+osSignalSelection_+"&&genTaus>0)*"+weight_),luminosity_*zttScale_*leg1Corr*tauID_,prefix);

    if(zShape.size()==0) {
      std::pair<float,float> zttShape       = createHistogramAndShifts(zttFile_,"ZTT",("("+preSelection+"&&"+trigSelection_+"&&"+osSignalSelection_+"&&genTaus>0)*"+weight_),luminosity_*zttScale_*leg1Corr*tauID_,prefix);
    }
    else
      {
	//create correction factor for MT and Pzeta cuts
	std::pair<float,float> dataWCut_Before       = createHistogramAndShifts(zShape,"ZTTWCut",("("+preSelection+")*embeddedWeight*__CORR__"),1.0,prefix);
	std::pair<float,float> zttShape       = createHistogramAndShifts(zShape,"ZTT",("("+preSelection+"&&"+osSignalSelection_+")*embeddedWeight*__CORR__"),1.0,prefix);

	std::pair<float,float> mcWCut_Before    = createHistogramAndShifts(zttFile_,"ZTTWCutMC",("("+preSelection+"&&"+trigSelection_+"&&genTaus>0)*"+weight_),luminosity_*zttScale_*leg1Corr*tauID_,prefix);
    ...
    //create TTbar
    std::pair<float,float> topYield       = createHistogramAndShifts(topFile_,"TT",("("+preSelection+"&&"+trigSelection_+"&&"+osSignalSelection_+")*"+weight_),luminosity_*leg1Corr*tauID_,prefix);
    //Create TTbar in Sideband
    std::pair<float,float> topYieldSdb    = createHistogramAndShifts(topFile_,"TT_SDB",("("+preSelection+"&&"+trigSelection_+"&&"+osWSelection_+")*"+weight_),luminosity_*leg1Corr*tauID_,prefix);

    std::pair<float,float> topSSYield       = createHistogramAndShifts(topFile_,"TTSS",("("+preSelection+"&&"+trigSelection_+"&&"+ssSignalSelection_+")*"+weight_),luminosity_*leg1Corr*tauID_,prefix);
    //Create TTbar in Sideband
    std::pair<float,float> topSSYieldSdb    = createHistogramAndShifts(topFile_,"TTSS_SDB",("("+preSelection+"&&"+trigSelection_+"&&"+ssWSelection_+")*"+weight_),luminosity_*leg1Corr*tauID_,prefix);
    ...
    std::pair<float,float> vvYieldSdb     = createHistogramAndShifts(vvFile_,"VV_SDB",("("+preSelection+"&&"+trigSelection_+"&&"+osWSelection_+")*"+weight_),luminosity_*leg1Corr*tauID_,prefix);

    std::pair<float,float> zlftYield;
    if(channel_=="muTau")
          zlftYield  = createHistogramAndShifts(zllFile_,"ZL",("("+preSelection+"&&"+trigSelection_+"&&"+osSignalSelection_+"&&genTaus==0&&genPt2>1)*"+weight_),luminosity_*leg1Corr*zlftFactor_*zttScale_,prefix,false);
    else
          zlftYield  = createHistogramAndShifts(zllFile_,"ZL",("("+preSelection+"&&"+trigSelection_+"&&"+osSignalSelection_+"&&genTaus==0&&genPt2>1)*(1/(__CORR__))*"+weight_),luminosity_*leg1Corr*zlftFactor_*zttScale_,prefix,false);

    std::pair<float,float> zjftYield      = createHistogramAndShifts(zllFile_,"ZJ",("("+preSelection+"&&"+trigSelection_+"&&"+osSignalSelection_+"&&genTaus==0&&genPt2<1)*"+weight_),luminosity_*leg1Corr*zttScale_,prefix);
    std::pair<float,float> zlftSSYield    = createHistogramAndShifts(zllFile_,"ZL_SS",("("+preSelection+"&&"+trigSelection_+"&&"+ssSignalSelection_+"&&genTaus==0&&genPt2>1)*"+weight_),luminosity_*leg1Corr*zttScale_,prefix);
    std::pair<float,float> zjftSSYield    = createHistogramAndShifts(zllFile_,"ZJ_SS",("("+preSelection+"&&"+trigSelection_+"&&"+ssSignalSelection_+"&&genTaus==0&&genPt2<1)*"+weight_),luminosity_*leg1Corr*zttScale_,prefix,false);
    std::pair<float,float> wShape         = createHistogramAndShifts(wFile42_,"W",("("+preSelection+"&&"+osSignalSelection_+")*"+weight_),luminosity_*leg1Corr,prefix,false);
    std::pair<float,float> wMCYield       = createHistogramAndShifts(wFile_,"WTMP",("("+preSelection+"&&"+trigSelection_+"&&"+osSignalSelection_+")*"+weight_),luminosity_*leg1Corr,prefix,false);
    ...
    std::pair<float,float> dataY         = createHistogramAndShifts(dataFile_,"data_obs","("+preSelection+"&&"+trigSelection_+"&&"+osSignalSelection_+")",scaleUp_,prefix);
    ...
    std::pair<float,float> dataYSdb     = createHistogramAndShifts(dataFile_,"data_obs_sdb","("+preSelection+"&&"+trigSelection_+"&&"+osWSelection_+")",scaleUp_,prefix);
    ...
    std::pair<float,float> wFactor = extractWFactor(wFile_,preSelection+"&&"+trigSelection_,prefix);
  ...
  BkgOutput runMinimalExtrapolation(std::string preSelection,std::string categorySelection,std::string prefix,float zExtrap_, float zExtrapErr_,float topExtrap,float topExtrapErr,BkgOutput inclusive, std::string zShape) {
    ...
    //create Z->tautau 
    std::pair<float,float> zttYield       = createHistogramAndShifts(zttFile_,"ZTTTMP",("("+preSelection+"&&"+trigSelection_+"&&"+osSignalSelection_+"&&"+categorySelection+"&&genTaus>0)*"+weight_),luminosity_*zttScale_*zExtrap*leg1Corr*tauID_,prefix);
    if(zShape.size()==0) {
      std::pair<float,float> zttShape       = createHistogramAndShifts(zttFile_,"ZTT",("("+preSelection+"&&"+trigSelection_+"&&"+osSignalSelection_+"&&"+categorySelection+"&&genTaus>0)*"+weight_),luminosity_*zttScale_*zExtrap*leg1Corr*tauID_,prefix);
    }
    else
      {
	printf("You have embedded samples.Calculating corrections on the fly overidding the crappy ones you gave me!\n");
	std::pair<float,float> zttShape       = createHistogramAndShifts(zShape,"ZTT",("("+preSelection+"&&"+categorySelection+"&&"+osSignalSelection_+")*embeddedWeight*__CORR__"),leg1Corr*tauID_,prefix,true);
	std::pair<float,float> zttShapePre    = createHistogramAndShifts(zShape,"ZTTPre",("("+preSelection+"&&"+osSignalSelection_+")*embeddedWeight*__CORR__"),leg1Corr*tauID_,prefix,true);

	//take into account the inclusive correction factor
	std::pair<float,float> zttYieldWCut       = createHistogramAndShifts(zttFile_,"ZTTWCut",("("+preSelection+"&&"+trigSelection_+"&&"+osSignalSelection_+"&&genTaus>0)*"+weight_),luminosity_*zttScale_*leg1Corr*tauID_,prefix);
    ...
    std::pair<float,float> topShape       = createHistogramAndShifts(topFile_,"TT",("("+relaxedSelection_+"&&"+trigSelection_+"&&"+osSignalSelection_+"&&"+categorySelection+")*"+weight_),luminosity_*leg1Corr*tauID_*topExtrap,prefix);      
    //create TTbar
    std::pair<float,float> topYield       = createHistogramAndShifts(topFile_,"TTTMP",("("+preSelection+"&&"+trigSelection_+"&&"+osSignalSelection_+"&&"+categorySelection+")*"+weight_),luminosity_*leg1Corr*tauID_*topExtrap,prefix);
    //Create TTbar in Sideband
    std::pair<float,float> topYieldSdb    = createHistogramAndShifts(topFile_,"TT_SDB",("("+preSelection+"&&"+trigSelection_+"&&"+osWSelection_+"&&"+categorySelection+")*"+weight_),luminosity_*leg1Corr*tauID_*topExtrap,prefix);
    ...
    //create Diboson in sideband
    std::pair<float,float> vvYieldSdb     = createHistogramAndShifts(vvFile_,"VV_SDB",("("+preSelection+"&&"+trigSelection_+"&&"+osWSelection_+"&&"+categorySelection+")*"+weight_),luminosity_*leg1Corr*tauID_,prefix);

    std::pair<float,float> zlftYield;
    if(channel_=="eleTau")
      zlftYield      = createHistogramAndShifts(zllFile_,"ZL",("("+preSelection+"&&"+trigSelection_+"&&"+osSignalSelection_+"&&"+categorySelection+"&&genTaus==0&&genPt2>1)*"+weight_),luminosity_*leg1Corr*zlftFactor_*zttScale_*zExtrap,prefix);
    else
      zlftYield      = createHistogramAndShifts(zllFile_,"ZL",("("+preSelection+"&&"+trigSelection_+"&&"+osSignalSelection_+"&&"+categorySelection+"&&genTaus==0&&genPt2>1)*"+weight_),luminosity_*leg1Corr*zlftFactor_*zttScale_*zExtrap,prefix);

    std::pair<float,float> zjftYield      = createHistogramAndShifts(zllFile_,"ZJ",("("+preSelection+"&&"+trigSelection_+"&&"+osSignalSelection_+"&&"+categorySelection+"&&genTaus==0&&genPt2<1)*"+weight_),luminosity_*leg1Corr*zttScale_*zExtrap,prefix);


    std::pair<float,float> zlftSSYield    = createHistogramAndShifts(zllFile_,"ZL_SS",("("+preSelection+"&&"+trigSelection_+"&&"+ssSignalSelection_+"&&"+categorySelection+"&&genTaus==0&&genPt2>1)*"+weight_),luminosity_*leg1Corr*zttScale_*zExtrap,prefix,false);
    std::pair<float,float> zjftSSYield    = createHistogramAndShifts(zllFile_,"ZJ_SS",("("+preSelection+"&&"+trigSelection_+"&&"+ssSignalSelection_+"&&"+categorySelection+"&&genTaus==0&&genPt2<1)*"+weight_),luminosity_*leg1Corr*zttScale_*zExtrap,prefix);
    std::pair<float,float> wMCYield       = createHistogramAndShifts(wFile_,"WMC",("("+preSelection+"&&"+trigSelection_+"&&"+osSignalSelection_+"&&"+categorySelection+")*"+weight_),luminosity_*leg1Corr,prefix);
    std::pair<float,float> wShape       = createHistogramAndShifts(wFile42_,"W",("("+preSelection+"&&"+categorySelection+"&&"+osSignalSelection_+")*"+weight_),luminosity_*leg1Corr,prefix,false);
    ...
    std::pair<float,float> dataY         = createHistogramAndShifts(dataFile_,"data_obs","("+preSelection+"&&"+trigSelection_+"&&"+osSignalSelection_+"&&"+categorySelection+")",scaleUp_,prefix);
    std::pair<float,float> dataYRounded  = std::make_pair(rintf(dataY.first),dataY.second);
    renormalizeHistogram(channel_+prefix,"data_obs",dataYRounded.first);
    std::pair<float,float> dataYield      = convertToPoisson(dataYRounded);
    ...
    std::pair<float,float> dataYSdb     = createHistogramAndShifts(dataFile_,"data_obs_sdb","("+preSelection+"&&"+trigSelection_+"&&"+osWSelection_+"&&"+categorySelection+")",scaleUp_,prefix);
    std::pair<float,float> dataYieldSdb      = convertToPoisson(dataYSdb);
    ...
    //Measure W factor from your corrected MC 
    std::pair<float,float> wFactor = extractWFactor(wFile_,preSelection+"&&"+trigSelection_+"&&"+categorySelection,prefix);
    printf("W extrapolation factor as measured in corrected MC = %f +- %f\n",wFactor.first,wFactor.second);
  ...
  BkgOutput runABCD(std::string preSelection,std::string catSelection,std::string prefix,float zExtrap,float zExtrapErr,float topExtrap,float topExtrapErr,std::string zShape = "") {
    ...
    //create Z->tautau 
    std::pair<float,float> zttYield       = createHistogramAndShifts(zttFile_,"ZTTTMP",("("+preSelection+"&&"+trigSelection_+"&&"+catSelection+"&&"+osSignalSelection_+"&&genTaus>0)*"+weight_),luminosity_*zttScale_*muID_*zExtrap*eleID_,prefix);

    if(zShape.size()==0) {
      std::pair<float,float> zttShape     = createHistogramAndShifts(zttFile_,"ZTT",("("+preSelection+"&&"+trigSelection_+"&&"+catSelection+"&&"+osSignalSelection_+"&&genTaus>0)*"+weight_),luminosity_*zttScale_*muID_*zExtrap*eleID_,prefix);
	output.ZTTCORR=zExtrap;
	output.dZTTCORR=zExtrapErr;
    }
    else {
	printf("You have embedded samples.Calculating corrections on the fly overidding the crappy ones you gave me!\n");
	std::pair<float,float> zttPreMC     = createHistogramAndShifts(zttFile_,"ZTTMCPre",("("+preSelection+"&&"+trigSelection_+"&&"+osSignalSelection_+"&&genTaus>0)*"+weight_),luminosity_*zttScale_*muID_*eleID_,prefix);
  ...
  BkgOutput runFullExtrapolation(std::string preSelection,std::string categorySelection, std::string prefix, BkgOutput inclusive,float zExtrap,float zExtrapErr,float wExtrap,float wExtrapErr,float topExtrap,float topExtrapErr,std::string zShape_="",std::string wShape_="") {
    ...
    std::pair<float,float> zttYield       = createHistogramAndShifts(zttFile_,"ZTTTMP",("("+preSelection+"&&"+trigSelection_+"&&"+categorySelection+"&&"+osSignalSelection_+"&&genTaus>0)*"+weight_),luminosity_*zttScale_*leg1Corr*tauID_*zExtrap,prefix,true);
	float corr = 1.0;
	
    if(zShape_.size()==0) {
      std::pair<float,float> zttShape       = createHistogramAndShifts(zttFile_,"ZTT",("("+preSelection+"&&"+trigSelection_+"&&"+categorySelection+"&&"+osSignalSelection_+"&&genTaus>0)*"+weight_),luminosity_*zttScale_*leg1Corr*tauID_*zExtrap,prefix,true);
	...
	printf("You have embedded samples.Calculating corrections on the fly overidding the crappy ones you gave me!\n");
	if(prefix=="_vbf")
	std::pair<float,float> zttShape       = createHistogramAndShifts(zShape_,"ZTT",("("+preSelection+"&&nJetsPt30>=2&&"+osSignalSelection_+")*embeddedWeight*__CORR__"),leg1Corr*tauID_,prefix,true);
	else
	std::pair<float,float> zttShape       = createHistogramAndShifts(zShape_,"ZTT",("("+preSelection+"&&"+categorySelection+"&&"+osSignalSelection_+")*embeddedWeight*__CORR__"),leg1Corr*tauID_,prefix,true);
	
	std::pair<float,float> zttEmbYield    = createHistogramAndShifts(zShape_,"ZTTTMP",("("+preSelection+"&&"+categorySelection+"&&"+osSignalSelection_+")*embeddedWeight*__CORR__"),leg1Corr*tauID_,prefix,true);

	std::pair<float,float> zttShapePre    = createHistogramAndShifts(zShape_,"ZTTPre",("("+preSelection+"&&"+osSignalSelection_+")*embeddedWeight*__CORR__"),leg1Corr*tauID_,prefix,true);

	std::pair<float,float> zttYieldWCut   = createHistogramAndShifts(zttFile_,"ZTTTMP",("("+preSelection+"&&"+trigSelection_+"&&"+osSignalSelection_+"&&genTaus>0)*"+weight_),luminosity_*zttScale_*leg1Corr*tauID_,prefix,false);
    ...
    if(prefix=="_vbf")
      std::pair<float,float> topShape       = createHistogramAndShifts(topFile_,"TT",("("+relaxedSelection_+"&& nJetsPt30>=2)*"+weight_),luminosity_*leg1Corr*tauID_,prefix,true,false);
    else if(prefix=="_btag_low" || prefix=="_btag_high")
    ...
    std::pair<float,float> topYield       = createHistogramAndShifts(topFile_,"TTTMP",("("+preSelection+"&&"+trigSelection_+"&&"+categorySelection+"&&"+osSignalSelection_+")*"+weight_),luminosity_*leg1Corr*tauID_*topExtrap,prefix);
    renormalizeHistogram(channel_+prefix,"TT",topYield.first);

    std::pair<float,float> topSSYield       = createHistogramAndShifts(topFile_,"TTSS",("("+preSelection+"&&"+trigSelection_+"&&"+categorySelection+"&&"+ssSignalSelection_+")*"+weight_),luminosity_*leg1Corr*tauID_*topExtrap,prefix);
	if(prefix=="_vbf")
		std::pair<float,float> vvShape        = createHistogramAndShifts(vvFile_,"VV",("("+relaxedSelection_+"&&"+osSignalSelection_+"&&nJetsPt30>=2)*"+weight_),luminosity_*leg1Corr*tauID_,prefix,true,false);
    else if(prefix=="_btag_low" || prefix=="_btag_high")
		std::pair<float,float> vvShape        = createHistogramAndShifts(vvFile_,"VV",("("+relaxedSelection_+"&&"+osSignalSelection_+"&&nJetsPt20==1)*"+weight_),luminosity_*leg1Corr*tauID_,prefix,true,false);
    else
		std::pair<float,float> vvShape        = createHistogramAndShifts(vvFile_,"VV",("("+relaxedSelection_+"&&"+osSignalSelection_+")*"+weight_),luminosity_*leg1Corr*tauID_,prefix,true,false);
    
    std::pair<float,float> vvYield        = createHistogramAndShifts(vvFile_,"VVTMP",("("+preSelection+"&&"+trigSelection_+"&&"+categorySelection+"&&"+osSignalSelection_+")*"+weight_),luminosity_*leg1Corr*tauID_,prefix,true,false);
    renormalizeHistogram(channel_+prefix,"VV",vvYield.first);
    
	std::pair<float,float> zllShape;
	
	if(prefix=="_vbf")
      zllShape      = createHistogramAndShifts(zttFile_,"ZLL",("("+preSelection+"&&"+trigSelection_+"&&"+osSignalSelection_+"&&genTaus==0&&nJetsPt30>=2)*"+weight_),luminosity_*leg1Corr*zttScale_,prefix,false,false);
	else if(prefix=="_btag_low" || prefix=="_btag_high")
      zllShape      = createHistogramAndShifts(zttFile_,"ZLL",("("+preSelection+"&&"+trigSelection_+"&&"+osSignalSelection_+"&&genTaus==0&&nJetsPt20>=1)*"+weight_),luminosity_*leg1Corr*zttScale_,prefix,false,false);
	else
      zllShape      = createHistogramAndShifts(zttFile_,"ZLL",("("+preSelection+"&&"+trigSelection_+"&&"+osSignalSelection_+"&&genTaus==0)*"+weight_),luminosity_*leg1Corr*zttScale_,prefix,false,false);

	std::pair<float,float> zllYield;	
	if(zShape_.size()==0)
	    zllYield      = createHistogramAndShifts(zttFile_,"ZLTMP",("("+preSelection+"&&"+trigSelection_+"&&"+osSignalSelection_+"&&"+categorySelection+"&&genTaus==0)*"+weight_),luminosity_*leg1Corr*zExtrap*zttScale_,prefix,false);
	else
	    zllYield      = createHistogramAndShifts(zttFile_,"ZLTMP",("("+preSelection+"&&"+trigSelection_+"&&"+osSignalSelection_+"&&genTaus==0)*"+weight_),luminosity_*leg1Corr*zExtrap*zttScale_*corr,prefix,false);
    ...
    std::pair<float,float> zllSSYield    = createHistogramAndShifts(zttFile_,"ZL_SS",("("+preSelection+"&&"+trigSelection_+"&&"+categorySelection+"&&"+ssSignalSelection_+"&&genTaus==0)*"+weight_),luminosity_*leg1Corr*zExtrap*zttScale_,prefix,false);

	std::pair<float,float> wYield;
    
     if(wShape_.size()==0 && !(prefix=="_btag_low" || prefix=="_btag_high")) {
       std::pair<float,float> wShape      = createHistogramAndShifts(wFile42_,"W",("("+relaxedSelection_+"&&"+categorySelection+")*"+weight_),luminosity_*leg1Corr*wExtrap*inclusive.WCORR,prefix,false,false);
       wYield      = createHistogramAndShifts(wFile_,"WTMP",("("+preSelection+"&&"+trigSelection_+"&&"+categorySelection+"&&"+osSignalSelection_+")*"+weight_),luminosity_*leg1Corr*wExtrap*inclusive.WCORR,prefix,false);
	   renormalizeHistogram(channel_+prefix,"W",wYield.first);
	   
       }
     else if(prefix=="_btag_low" || prefix=="_btag_high"){
     
      std::pair<float,float> wShape      = createHistogramAndShifts(wFile42_,"W",("("+preSelection+"&&"+osSignalSelection_+"&&nJetsPt20==1)*"+weight_),luminosity_*leg1Corr*wExtrap,prefix,false,false);
      std::pair<float,float> dataYWSDB   = createHistogramAndShifts(dataFile_,"dataWSDB","("+preSelection+"&&"+categorySelection+"&&"+osWSelection_+"&&"+trigSelection_+")",scaleUp_,prefix);
   	  std::pair<float,float> vvYieldSDB  = createHistogramAndShifts(vvFile_,"VVSDB",("("+preSelection+"&&"+categorySelection+"&&"+osWSelection_+"&&"+trigSelection_+")*"+weight_),luminosity_*leg1Corr*tauID_,prefix,true,false);
	  std::pair<float,float> topYieldSDB = createHistogramAndShifts(topFile_,"TTSDB",("("+preSelection+"&&"+categorySelection+"&&"+osWSelection_+"&&"+trigSelection_+")*"+weight_),luminosity_*leg1Corr*tauID_*topExtrap,prefix);

      std::pair<float,float> wFactor = extractWFactor(wFile42_,preSelection+"&&nJetsPt20>=2",prefix);
	  

      std::pair<float,float> osWHigh = std::make_pair(TMath::Nint(dataYWSDB.first-topYieldSDB.first-vvYieldSDB.first),
						      sqrt(dataYWSDB.second*dataYWSDB.second+topYieldSDB.second*topYieldSDB.second+vvYieldSDB.second*vvYieldSDB.second));	  
	  ...
	  std::pair<float,float> wShape      = createHistogramAndShifts(wShape_,"W",("(nJetsPt30>=2 && "+preSelection+"&&"+osSignalSelection_+")*"+weight_),luminosity_*leg1Corr,prefix,false,false);
      std::pair<float,float> dataYWSDB   = createHistogramAndShifts(dataFile_,"dataWSDB","("+preSelection+"&&"+categorySelection+"&&"+osWSelection_+"&&"+trigSelection_+")",scaleUp_,prefix);
   	  std::pair<float,float> vvYieldSDB  = createHistogramAndShifts(vvFile_,"VVSDB",("("+preSelection+"&&"+categorySelection+"&&"+osWSelection_+"&&"+trigSelection_+")*"+weight_),luminosity_*leg1Corr*tauID_,prefix,true,false);
	  std::pair<float,float> topYieldSDB = createHistogramAndShifts(topFile_,"TTSDB",("("+preSelection+"&&"+categorySelection+"&&"+osWSelection_+"&&"+trigSelection_+")*"+weight_),luminosity_*leg1Corr*tauID_*topExtrap,prefix);

      std::pair<float,float> wFactor = extractWFactor(wShape_,"nJetsPt30>=2&&"+preSelection+"&&"+trigSelection_,prefix);
    ...
    std::pair<float,float> dataY         = createHistogramAndShifts(dataFile_,"data_obs","("+preSelection+"&&"+trigSelection_+"&&"+categorySelection+"&&"+osSignalSelection_+")",scaleUp_,prefix);
    std::pair<float,float> dataYRounded  = std::make_pair(rintf(dataY.first),dataY.second);
    renormalizeHistogram(channel_+prefix,"data_obs",dataYRounded.first);
    std::pair<float,float> dataYield      = convertToPoisson(dataYRounded);
	...
	if(prefix!="_vbf"){
		...
		dataQCDShape = createHistogramAndShifts(dataFile_,"QCD",vbfQCDSelection_,scaleUp_,prefix,false,false);
		std::pair<float,float> dataQCDControl = createHistogramAndShifts(dataFile_,"QCDC1",qcdSelection_+"&&"+categorySelection,scaleUp_,prefix,false,false);
		std::pair<float,float> dataQCDControlInc = createHistogramAndShifts(dataFile_,"QCDC2",qcdSelection_,scaleUp_,prefix,false,false);	
    ...
    if(channel_=="eleTau"&&prefix!="_SM3") {
      std::pair<float,float> zllQCDControl = createHistogramAndShifts(zttFile_,"ZLLQCD","("+qcdSelection_+"&&"+categorySelection+"&&genTaus==0)*"+weight_,luminosity_*leg1Corr*zExtrap*zttScale_,prefix,false,false);
      std::pair<float,float> zllQCDControlInc = createHistogramAndShifts(zttFile_,"ZLLQCDC","("+qcdSelection_+"&&genTaus==0)*"+weight_,luminosity_*leg1Corr*zExtrap*zttScale_,prefix,false,false);

      qcdPass-=zllQCDControl.first;
      if(qcdPass<0) qcdPass=0.0001;
      qcdAll-=zllQCDControlInc.first;
      if(qcdAll<0) qcdAll=0.0001;
      subtractHistogram(channel_+prefix,"QCDC1","ZLLQCD");
      subtractHistogram(channel_+prefix,"QCDC2","ZLLQCDC");
    }
  ...
  std::pair<float,float> extractWFactor(std::string file,std::string preselection,std::string postfix) {
    TFile *f  = new TFile (file.c_str());
    TTree *t = (TTree*)f->Get((channel_+"EventTree/eventTree").c_str());

	std::pair<float,float> high;
	std::pair<float,float> low;
	high = makeHistogram(t,channel_+postfix,"W_High",("("+preselection+"&&"+osWSelection_+")*"+weight_).c_str());
	low = makeHistogram(t,channel_+postfix,"W_Low",("("+preselection+"&&"+osSignalSelection_+")*"+weight_).c_str());
