...
theFile  = TFile("./allFits/%s/mlfit_%s_full_h_Wbb%s_%s_%s%s.root"%(version,runname,wflav,ttsample,lep,loc))
biasFile = TFile("./allFits/%s/mlfit_%s_full_h_Wbb%s_%s_%s%s_fitbias.root"%(version,runname,wflav,ttsample,lep,loc))

#where = './allFits/plots_%s'%(version)
#
#print("./allFits/roots_%s/%s.root"%(version,sys.argv[3]))
#dataFile_tt  = TFile("./allFits/roots_%s/%s.root"%(version,sys.argv[3]))
#theFile  = TFile("./allFits/roots_%s/mlfit_%s_full_h_Wbb%s_%s_%s%s.root"%(version,runname,wflav,ttsample,lep,loc))
#biasFile = TFile("./allFits/roots_%s/mlfit_%s_full_h_Wbb%s_%s_%s%s_fitbias.root"%(version,runname,wflav,ttsample,lep,loc))
...
for shape in shapes:
 ...
 h_Wbb = theFile.Get("%s/%s/h_Wbb%s%s"%(shape,sample,wflav,loc))
 h_Wbb.SetName("h_Wbb")
 h_Wbb.SetFillColor( c_wbb )
 h_Wbb.SetFillStyle( fillStyle )
 h_Wbb.Rebin( rebin )
 h_Wbb.Scale( sf_wbb )
 h_Wbb.Draw("GOFF")
 if (shp=="PreFit" and sample=="UUbb"):
  sizes_wbb_prefit.append(h_Wbb.Integral())
  sizes_wbb_part_prefit.append(h_Wbb.Integral(I,F))
  Wbb_prefit_size=h_Wbb.Integral(-1,-1)
 elif (shp=="PreFit" and sample=="TTbr"): 
  sizes_tt_prefit.append(h_Wbb.Integral())
  sizes_tt_part_prefit.append(h_Wbb.Integral(I,F))
 elif (shp=="Fitted" and sample=="UUbb"):
  sizes_wbb_fitted.append(h_Wbb.Integral())
  sizes_wbb_part_fitted.append(h_Wbb.Integral(I,F))
  Wbb_fitted_size=h_Wbb.Integral(-1,-1)
 elif (shp=="Fitted" and sample=="TTbr"): 
  sizes_tt_fitted.append(h_Wbb.Integral())
  sizes_tt_part_fitted.append(h_Wbb.Integral(I,F))
 sample_names.append("W+bb    ")
 
 h_Wcc = theFile.Get("%s/%s/h_Wcc%s"%(shape,sample,loc))
 ...
 h_Wcc.Draw("GOFF")
 if (shp=="PreFit" and sample=="UUbb"):
  sizes_wbb_prefit.append(h_Wcc.Integral())
  sizes_wbb_part_prefit.append(h_Wcc.Integral(I,F))
 elif (shp=="PreFit" and sample=="TTbr"): 
  sizes_tt_prefit.append(h_Wcc.Integral())
  sizes_tt_part_prefit.append(h_Wcc.Integral(I,F))
 elif (shp=="Fitted" and sample=="UUbb"):
  sizes_wbb_fitted.append(h_Wcc.Integral())
  sizes_wbb_part_fitted.append(h_Wcc.Integral(I,F))
 elif (shp=="Fitted" and sample=="TTbr"): 
  sizes_tt_fitted.append(h_Wcc.Integral())
  sizes_tt_part_fitted.append(h_Wcc.Integral(I,F))
 sample_names.append("W+cc    ")
 ...
 if (shp=="PreFit" and sample=="UUbb"):
  ...
  sizes_tt_part_prefit.append(
   h_TTbar.Integral(I,F) )
  TTbar_prefit_size=h_TTbar.Integral(-1,-1)
 elif (shp=="Fitted" and sample=="UUbb"):
  ...
  sizes_tt_part_fitted.append(
   h_TTbar.Integral(I,F) )
  TTbar_fitted_size=h_TTbar.Integral(-1,-1)
 ...
 h_Wl = theFile.Get("%s/%s/h_Wl%s"%(shape,sample,loc))
 ...
 h_Wl.Draw("GOFF")
 if (shp=="PreFit" and sample=="UUbb"):
  sizes_wbb_prefit.append(h_Wl.Integral())
  sizes_wbb_part_prefit.append(h_Wl.Integral(I,F))
 elif (shp=="PreFit" and sample=="TTbr"): 
  sizes_tt_prefit.append(h_Wl.Integral())
  sizes_tt_part_prefit.append(h_Wl.Integral(I,F))
 elif (shp=="Fitted" and sample=="UUbb"):
  sizes_wbb_fitted.append(h_Wl.Integral())
  sizes_wbb_part_fitted.append(h_Wl.Integral(I,F))
 elif (shp=="Fitted" and sample=="TTbr"): 
  sizes_tt_fitted.append(h_Wl.Integral())
  sizes_tt_part_fitted.append(h_Wl.Integral(I,F))
 sample_names.append("W+light ")
 ...
 leg.AddEntry(h_Wbb,"W+b#bar{b}","f")
 leg.AddEntry(h_Wcc,"W+c#bar{c}","f")
 leg.AddEntry(h_Wl,"W+udscg","f")
 ...
 c.Print(where+"/Wbb%s_%s_%s_%s_%s_%s%s.png"%(wflav,ttsample,sample,shp,lep,runname,loc))
 print(  where+"/Wbb%s_%s_%s_%s_%s_%s%s.png"%(wflav,ttsample,sample,shp,lep,runname,loc))
 c.Clear()
...
#
if ttsample=="ttme": ttname = "multi-lepton"
if ttsample=="ttjjj": ttname = "multi-jet"
...
if sample=="TTbr": 
 ...
 log = open(where+"/Wbb%s_%s_%s%s.tex"%(wflav,ttsample,lep,loc),'w')
 ...
 logb = open(where+"/Wbb%s_%s_%s%s_b.tex"%(wflav,ttsample,lep,loc),'w')
