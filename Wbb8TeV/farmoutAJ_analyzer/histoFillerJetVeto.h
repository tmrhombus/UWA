#ifndef histoFillerJetVeto_h
#define histoFillerJetVeto_h
...
public :
   ...
   TH1F hists_nAllJets[7][2][2][7][5];
   TH1F hists_nCleanJets[7][2][2][7][5];
   TH1F hists_nGoodJets[7][2][2][7][5];
   TH1F hists_nFwdJets[7][2][2][7][5];
   ...
   histoFillerJetVeto(TTree *tree=0, Bool_t isMC=kTRUE);
   virtual ~histoFillerJetVeto();
...
#ifdef histoFillerJetVeto_cxx
histoFillerJetVeto::histoFillerJetVeto(TTree *tree, Bool_t isMC) : fChain(0) 
{
...
histoFillerJetVeto::~histoFillerJetVeto()
{
   if (!fChain) return;
...
Int_t histoFillerJetVeto::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t histoFillerJetVeto::LoadTree(Long64_t entry)
{
...
void histoFillerJetVeto::Init(TTree *tree, Bool_t isMC)
{
     ...
     for(unsigned int k=0; k<QCD.size(); ++k){
      for(unsigned int l=0; l<Syst.size(); ++l){
       for(unsigned int m=0; m<WFlav.size(); ++m){
        ...
        TString histoname_nAllJets = "h_"+PSpace[i]+"_"+EMu[j]+"_"+QCD[k]+"_nAllJets"+Syst[l]+WFlav[m];
        TString histoname_nCleanJets = "h_"+PSpace[i]+"_"+EMu[j]+"_"+QCD[k]+"_nCleanJets"+Syst[l]+WFlav[m];
        TString histoname_nGoodJets = "h_"+PSpace[i]+"_"+EMu[j]+"_"+QCD[k]+"_nGoodJets"+Syst[l]+WFlav[m];
        TString histoname_nFwdJets = "h_"+PSpace[i]+"_"+EMu[j]+"_"+QCD[k]+"_nFwdJets"+Syst[l]+WFlav[m];
        ...
        hists_nAllJets[i][j][k][l][m].Clear();
        hists_nAllJets[i][j][k][l][m] = TH1F(histoname_nAllJets, "nAllJets", 10, 0., 10.);
        hists_nAllJets[i][j][k][l][m].Sumw2();                                               

        hists_nGoodJets[i][j][k][l][m].Clear();
        hists_nGoodJets[i][j][k][l][m] = TH1F(histoname_nGoodJets, "nGoodJets", 10, 0., 10.);
        hists_nGoodJets[i][j][k][l][m].Sumw2();                                               

        hists_nCleanJets[i][j][k][l][m].Clear();
        hists_nCleanJets[i][j][k][l][m] = TH1F(histoname_nCleanJets, "nCleanJets", 10, 0., 10.);
        hists_nCleanJets[i][j][k][l][m].Sumw2();                                               

        hists_nFwdJets[i][j][k][l][m].Clear();
        hists_nFwdJets[i][j][k][l][m] = TH1F(histoname_nFwdJets, "nFwdJets", 10, 0., 10.);
        hists_nFwdJets[i][j][k][l][m].Sumw2();                                               
...
Bool_t histoFillerJetVeto::Notify()
{
...
void histoFillerJetVeto::Show(Long64_t entry)
{
...
double histoFillerJetVeto::dPhi(double phi1, double phi2){
 double dphi;
...
double histoFillerJetVeto::dEta(double eta1, double eta2){
 double deta;
 deta = std::abs(eta1-eta2);
 return deta;
}

Bool_t histoFillerJetVeto::FillHistograms(
  int i, int j, int k, int l, int m, 
  double metval, double metphival, double mt, 
  double lep_pt, double lep_eta, double lep_phi, double lep_mass,
  double dilep_pt, double dilep_eta, double dilep_phi, double dilep_mass,
 ...
 hists_nAllJets[i][j][k][l][m].Fill(nrAllJets,weight);
 hists_nGoodJets[i][j][k][l][m].Fill(nrGoodJets,weight);
 hists_nCleanJets[i][j][k][l][m].Fill(nrCleanJets,weight);
 hists_nFwdJets[i][j][k][l][m].Fill(nrFwdJets,weight);
...
Bool_t histoFillerJetVeto::WriteHistograms(int i, int j, int k, int l, int m){
 hists_nAllJets[i][j][k][l][m].Write();
 hists_nGoodJets[i][j][k][l][m].Write();
 hists_nCleanJets[i][j][k][l][m].Write();
 hists_nFwdJets[i][j][k][l][m].Write();
...
Bool_t histoFillerJetVeto::DeleteHistograms(int i, int j, int k, int l, int m){
 hists_nAllJets[i][j][k][l][m].Delete();
 hists_nGoodJets[i][j][k][l][m].Delete();
 hists_nCleanJets[i][j][k][l][m].Delete();
 hists_nFwdJets[i][j][k][l][m].Delete();
...
Bool_t histoFillerJetVeto::fillHistWriter(
...
Bool_t histoFillerJetVeto::writeHistWriter( int pspace_nr, int syst_nr, Bool_t isW ){
...
#endif // #ifdef histoFillerJetVeto_cxx
