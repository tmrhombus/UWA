/********************************
Instantiates histoFillerJetVeto.C with
the correct chain and naming
for a definable set of parameters

Usage: 
root -l -b
.L PSANDSHIFT_callHistoFillerJetVeto.cc
PSANDSHIFT_callHistoFillerJetVeto()

root -l -b -q PSANDSHIFT_callHistoFillerJetVeto.cc
...
void PSANDSHIFT_callHistoFillerJetVeto()
{
 int error = 0;
 gROOT->ProcessLine(".L histoFillerJetVeto.C++", &error);
 if(error!=0){std::cerr<<"ERROR LOADING histoFillerJetVeto.C"<<std::endl;}
