...
def applyDefaultSelectionsPT(process,jets,muons,electrons):
  ...
  electronAntiIso  = electronIso+' > 0.15 '  # Wbb, CestPi, Earth, Neptune, Mercury
  ...
  #muonAntiIso  = muonIso+' > 0.3 '  # VV, CestPiVV, Venus
  muonAntiIso  = muonIso+' > 0.20 '  # Wbb, CestPi, Earth, Neptune, Mercury
...
def IVF(process):
  ...
  process.LCProducer.jetSource= cms.InputTag("selectedPatJetsAK5chsPF")
  #process.LCProducer.jetSource= cms.InputTag("ak5PFJets")
  process.LCProducer.jetGenSource= cms.InputTag("ak5GenJetsNoNu")
