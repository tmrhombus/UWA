...
template <class T>
class LeptonMetSorterByLeadingPt : public edm::EDProducer {
   public:
  explicit LeptonMetSorterByLeadingPt(const edm::ParameterSet& iConfig):
    src_(iConfig.getParameter<edm::InputTag>("src"))
     {
       produces<std::vector<T> >();
     }

  ~LeptonMetSorterByLeadingPt() {}
  ...
  public:
    ...
    {
      return (t1.lepton()->pt()) > (t2.lepton()->pt());
    } 
};
