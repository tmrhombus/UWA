...
//                " phi: "<<jet.phi()<<std::endl;
	...
	for(unsigned int j=0;j<jet.getPFConstituents().size();++j) {
	  ...
	  if(pfcand.isNonnull()){
	    ...
	    //printf("pfcand: %d\n",pfcand);
	    if(abs(pfcand->pdgId())==211&&pfcand->pt()>1){ ///create charged particle collection
	      chargedParticles.push_back(pfcand);
	    }
	  
	    if(abs(pfcand->pdgId())==13&&pfcand->pt()>3){//create muon collection
	      muons.push_back(pfcand);
	    }
	  
	    if(abs(pfcand->pdgId())==11&&pfcand->pt()>1){//no one likes electrons
...
//	////adding Dpm code here--> despite what others want...
//	double massDpm = 0;
