...
class GenStudyAndrea : public edm::EDFilter {

public:
 GenStudyAndrea (const edm::ParameterSet &);
...
GenStudyAndrea::GenStudyAndrea( const ParameterSet & cfg ) :
...
void GenStudyAndrea::endJob() {
 ...
 cout<<"Lepton PDGID = "<<nNoTaus <<" - "<<inputCrossSection_/nNUP*nNoTaus<<endl ;
 ...
 h1_["Selection"]->SetBinContent(4,nNoTaus);
 ...
 h1_["CrossSection"]->SetBinContent(4,inputCrossSection_/nNUP*nNoTaus);
...
bool GenStudyAndrea::filter (Event & ev, const EventSetup &) {
nall++;

 double eventNumber=ev.id().event();
 double lumi=ev.id().luminosityBlock();
 ...
 if(abs(genpart.pdgId())!=13) continue;
// if(genpart.pt()<ptCutLepton_ || fabs(genpart.eta())>=etaCutLepton_) continue;
 if(genpart.Type()!=1) continue;
 countLeptons++;
 countHardLeptons++;
 if(genpart.pt()<ptmax) continue;
 leptonIndex=i;
 ptmax=genpart.pt();
 }
 if(leptonIndex==-1) { return false; }
...
// n2BHadrons++;
 ...
 printf("%6.0d (%5.0d) | %11.0f %6.f | %5.2f %4.3f %4.0d %4.0d \t| %5.2f %4.3f %5.2f %5.4f %5.f %5.f\t| %5.2f %4.3f %5.2f %5.4f %5.f %5.f\t| %5.2f %4.3f | \n",nall,n2TaggedJetsV2,eventNumber,lumi,lepton.pt(),lepton.eta(),lepton.pdgId(),lepton.Type(),jet1.pt(),jet1.eta(),jet1.userFloat("leadJetBHadronPt"),jet1.userFloat("minDRBHadron"),jet1.userFloat("leadJetBHadronPdgId"),jet1.userFloat("originalBAncestorAndrea"),jet2.pt(),jet2.eta(),jet2.userFloat("leadJetBHadronPt"),jet2.userFloat("minDRBHadron"),jet2.userFloat("leadJetBHadronPdgId"),jet2.userFloat("originalBAncestorAndrea"),MET,wmt);
...
/*
 cout<<"Bs?"<<BHadrons->size()<<endl;

 if(BHadrons->size()>=1) {
 SimBHadron genB = BHadrons->at(0);
 cout<<genB.pt()<<" "<<genB.eta()<<" "<<genB.pdgId()<<" "<<genB.status()<<" "<<genB.numberOfDaughters()<<endl;
 }

 if(BHadrons->size()>=2) {
 SimBHadron genB2 = BHadrons->at(1);
 cout<<genB2.pt()<<" "<<genB2.eta()<<" "<<genB2.pdgId()<<" "<<genB2.status()<<" "<<genB2.numberOfDaughters()<<endl;
 }
*/
...
DEFINE_FWK_MODULE(GenStudyAndrea);
